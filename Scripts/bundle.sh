...
# app in a file called DynamicIsland-.dmg.
if [ -z "$VERSION" ]; then
...
ENTITLEMENTS="$ROOT/build/DynamicIsland.entitlements"
cp "$ROOT/Resources/DynamicIsland.entitlements" "$ENTITLEMENTS"
TEAM_ID="$(printf '%s' "$SIGN_IDENTITY" | sed -n 's/.*(\([A-Z0-9]\{10\}\))$/\1/p')"
if [ -n "$TEAM_ID" ]; then
    echo "==> keychain access group: $TEAM_ID.dev.dynamicisland.app"
    /usr/libexec/PlistBuddy \
        -c "Add :keychain-access-groups array" \
        -c "Add :keychain-access-groups:0 string $TEAM_ID.dev.dynamicisland.app" \
...
if [ -f "$APP/Contents/Resources/libdynamicislandmedia.dylib" ]; then
    ...
    codesign "${NESTED_ARGS[@]}" --sign "$SIGN_IDENTITY" \
        "$APP/Contents/Resources/libdynamicislandmedia.dylib" || {
