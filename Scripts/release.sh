...
#
# ISLA_ADHOC=1 publishes the same gated build without Developer ID signing or
# notarization, for as long as there is no Developer ID to sign with: every
# gate still runs, the app keeps its ad-hoc signature, and the release notes
# carry the one command a downloaded unnotarized app needs.
#
# ISLA_RELEASE_REMOTE names the git remote the tag goes to (default origin).
# On a machine where origin is a local mirror, point it at the GitHub remote.
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
ADHOC="${ISLA_ADHOC:-0}"
REMOTE="${ISLA_RELEASE_REMOTE:-origin}"
...
# The tag has to name a commit the remote already has. This used to ask the
# branch's upstream, which a release made from a detached checkout of the
# merge commit does not have — and a missing upstream once let the check pass
# vacuously. Asking the remote's branches directly covers both.
[ -n "$(git branch -r --contains HEAD --list "$REMOTE/*")" ] || \
    fail "HEAD is on no $REMOTE branch; push it first so the tag names a commit on the remote"
...
if [ "$ADHOC" = "1" ]; then
    echo "==> ad-hoc release: no Developer ID signature, no notarization"
...
rollback_tag() {
    ...
    git push --quiet --delete "$REMOTE" "$TAG" 2>/dev/null || true
    git tag -d "$TAG" >/dev/null 2>&1 || true
}
trap 'rollback_tag' ERR

echo "==> GitHub release"
REPO="${ISLA_RELEASE_REPO:-$(gh repo view --json nameWithOwner --jq '.nameWithOwner')}"
